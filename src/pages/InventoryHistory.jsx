...
import { LogoutOutlined, UserOutlined, AppstoreOutlined, UnorderedListOutlined, SettingOutlined, ShopOutlined, HistoryOutlined, SearchOutlined, ReloadOutlined, DownloadOutlined, SwapRightOutlined } from '@ant-design/icons';
...
const InventoryHistory = () => {
    ...
    const fetchLogs = async (adminAuth, custName) => {
        ...
        if (!adminAuth && custName) {
            query = query.eq('customer_name', custName);
        }
    ...
    const handleDownloadExcel = () => {
        const excelData = getFilteredData().map(item => ({
            '일시': new Date(item.created_at).toLocaleString(),
            '구분': item.reason,
            '고객사': item.customer_name,
            '상품명': item.product_name,
            '변경전': item.previous_quantity,
            '변동': item.change_quantity,
            '변경후': item.new_quantity,
            '이전위치': item.previous_location || '-',
            '현재위치': item.new_location || '-',
            '작업자': item.changed_by
        }));
    ...
    const columns = [
        ...
        { 
            ...
            render: (_, r) => (
                <span>
                    <span style={{color: '#999'}}>{r.previous_quantity}</span>
                    <SwapRightOutlined style={{margin: '0 8px', color: '#ccc'}} />
                    <span style={{fontWeight:'bold', color: r.change_quantity > 0 ? 'blue' : (r.change_quantity < 0 ? 'red' : 'black')}}>
                        {r.new_quantity} 
                        ({r.change_quantity > 0 ? `+${r.change_quantity}` : r.change_quantity})
                    </span>
                </span>
            )
        },
        { 
            ...
            render: (_, r) => (r.previous_location !== r.new_location && r.new_location) 
                ? <span style={{fontSize:'12px'}}>{r.previous_location || '(없음)'} <SwapRightOutlined /> <Tag color="blue">{r.new_location}</Tag></span> 
